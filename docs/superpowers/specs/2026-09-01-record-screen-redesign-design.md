...
레퍼런스는 "상단 연함 → 하단 진함"이고 사용자가 그대로 가기로 결정했다. 다만 레퍼런스의 하단
색(대략 `orange-500`) 위에 흰 텍스트를 얹으면 대비가 약 3.6:1로 WCAG AA(4.5:1) 미달이다.
직전 다크 테마 슬라이스에서 정확히 이 문제로 한 번 되돌린 이력이 있으므로, 하단을 `orange-700`
(#c2410c, 흰색 대비 약 5.1:1)까지 내려 레퍼런스의 방향성은 살리되 대비 기준은 지킨다.

텍스트 색은 자기가 앉는 구간 기준으로 갈린다:

| 영역 | 배경 구간 | 텍스트 | 대비 |
|---|---|---|---|
| 상단 바 (뒤로 / 타이핑으로) | `orange-100` #ffedd5 | `text-slate-800` | ~12:1 |
| 상태 라벨 (듣고 있습니다) | `orange-200`대 | `text-slate-800` | ~10:1 |
| 파형 | `orange-400`대 | `fill-white/25~60` | 장식 요소, 대비 기준 미적용 |
| 타이머 / 대본 / 버튼 라벨 | `orange-600~700` | `text-white` | ~5.1:1 |
...
가운데 버튼이 녹음 상태에 따라 3상태를 가진다:

| 상태 | 왼쪽 (h-11 w-11) | 가운데 (h-28 w-28) | 오른쪽 (h-11 w-11) |
|---|---|---|---|
| 녹음 중 (`speech.isRecording`) | `⏸` 일시정지 | `✓` 완료 | `🗑` 삭제 |
| 정지됨 | `▶` 이어 녹음 | `넘어가기` | `🗑` 삭제 |

- `✓` → `speech.stop()` + `micLevel.stop()`. **화면 전환 없음.** 타이머 멈추고 대본은 남는다
- `넘어가기` → `setStep('details')`. `canSubmitRecord(speech.transcript)`가 false면 버튼이
  `disabled`이고 그 아래에 `먼저 녹음해주세요` 힌트를 보여준다.
  (현재 `stopVoiceAndContinue`는 조건 미달 시 조용히 아무것도 안 해서 사용자가 이유를 알 수 없다 —
  이 기회에 고친다)
- `▶` → `resumeVoiceRecording()` (기존 함수 그대로). 누르면 가운데는 다시 `✓`
- `🗑` → `cancelVoice()` (기존 함수 그대로) + 타이머 리셋
- 상단 우측 `⌨` → `switchVoiceToTyping()` (기존 함수 그대로)

`stopVoiceAndContinue`는 "정지 + 다음 단계"를 한 번에 하던 함수라 둘로 쪼갠다:
`stopVoiceRecording()`(정지만) / `goToDetailsFromVoice()`(다음 단계만).
